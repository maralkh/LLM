...
"""
Standalone training script for LLaMA with MoE
Usage: python train_llama_moe.py --config config.yaml --data data_path
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train LLaMA with MoE")
    ...
    parser.add_argument("--model-size", type=str, default="7b", choices=["7b"], help="Model size")
    parser.add_argument("--use-moe", action="store_true", help="Use Mixture of Experts")
    parser.add_argument("--moe-layers", nargs="+", type=int, default=[4, 8, 12, 16], help="MoE layer indices")
    ...
    # Create model
    if args.use_moe:
        model = create_llama_moe_7b(moe_layers=args.moe_layers, num_experts=args.num_experts)
    else:
        from training_infra.models.llama import create_llama_7b
        model = create_llama_7b()
...
'''
    ...
    with open("train_llama_moe.py", "w") as f:
        f.write(script_content)
    
    print("✅ Created standalone training script: train_llama_moe.py")
...
def create_inference_script():
    ...
    script_content = '''#!/usr/bin/env python3
"""
Standalone inference script for LLaMA models
Usage: python inference.py --model model.pt --prompt "Your prompt here"
"""

import argparse
import torch

from training_infra.inference.engine import generate_text, GenerationConfig
from training_infra.inference.sampling import SamplingConfig
...
'''
    ...
    print("✅ Created standalone inference script: inference.py")